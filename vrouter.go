...
func NewVrouter(agent *OfnetAgent, rpcServ *rpc.Server) *Vrouter {
	...
	vrouter.portDscpFlowDb = make(map[uint32]*ofctrl.Flow)
...
func (self *Vrouter) AddDscp(endpoint OfnetEndpoint, dscp uint8) error {
...
func (self *Vrouter) AddLocalEndpoint(endpoint OfnetEndpoint, dscp uint8) error {
...
//UpdateEpg creates a new dscp flow.
func (self *Vrouter) UpdateEpg(endpoint OfnetEndpoint, dscp uint8) error {

	log.Infof("received dscp update for port: %d  dscp: %d to be updated to: %d", endpoint.PortNo, endpoint.DSCP, dscp)
...
func (self *Vrouter) RemoveDscpFlow(endpoint OfnetEndpoint) error {
	portVlanDscp := self.portDscpFlowDb[endpoint.PortNo]
	if portVlanDscp != nil {
...
func (self *Vrouter) RemoveLocalEndpoint(endpoint OfnetEndpoint) error {
	...
	if endpoint.DSCP != 0 {
		log.Infof("Received dscp delete:")
		err := self.RemoveDscpFlow(endpoint)
		if err != nil {
			return err
		}
